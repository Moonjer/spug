...
import {Icon, message, Modal, Popover, Table, Tag} from 'antd';
            ...
            switch (info.status) {
                case '-3':
                    return <Action>
                        ...
                        <Action.Link auth="deploy.request.do"
                                     to={`/deploy/do/ext${info['app_extend']}/${info.id}`}>发布</Action.Link>
                        <Action.Button
                            auth="deploy.request.do"
                ...
                case '-1':
                    return <Action>
                        <Action.Button auth="deploy.request.edit"
                                       onClick={() => store.showForm(info)}>编辑</Action.Button>
                        <Action.Button auth="deploy.request.del"
                                       onClick={() => this.handleDelete(info)}>删除</Action.Button>
                    </Action>;
                case '0':
                    return <Action>
                        <Action.Button auth="deploy.request.approve"
                                       onClick={() => store.showApprove(info)}>审核</Action.Button>
                        <Action.Button auth="deploy.request.edit"
                                       onClick={() => store.showForm(info)}>编辑</Action.Button>
                        <Action.Button auth="deploy.request.del"
                                       onClick={() => this.handleDelete(info)}>删除</Action.Button>
