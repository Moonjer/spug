...
import {Button, Col, Form, Icon, Input, message, Modal, Select} from 'antd';
               ...
               href="https://spug.dev/docs/install-error/#%E6%96%B0%E5%BB%BA%E5%B8%B8%E8%A7%84%E5%8F%91%E5%B8%83%E7%94%B3%E8%AF%B7-git-clone-%E9%94%99%E8%AF%AF">clone 失败？</a>
                        ...
                        <Col span={19}>
                            <Input.Group compact>
                                ...
                                <Select
                                    ...
                                    {git_type === 'branch' ? (
                                        Object.entries(branches || []).map(([key, b]) => <Select.Option key={b.id}
                                                                                                        value={b.id}>{b.name}</Select.Option>)
                                    ) : (
                                        Object.entries(tags || []).map(([tag, info]) => (
                                            <Select.Option key={tag}
                                                           value={tag}>{`${tag} ${info.author} ${info.message}`}</Select.Option>
                                        ))
                                    )}
                                </Select>
                            </Input.Group>
                        ...
                        <Col span={4} offset={1} style={{textAlign: 'center'}}>
                            {fetching ? <Icon type="loading" style={{fontSize: 18, color: '#1890ff'}}/> :
                                <Button type="link" icon="sync" disabled={fetching}
                                        onClick={this.fetchVersions}>刷新</Button>
                            }
                        </Col>
                    </Form.Item>
                    {/*{git_type === 'branch' && (*/}
