...
import {Divider, Icon, message, Modal, Table, Tag} from 'antd';
...
class ComTable extends React.Component {
    ...
    componentDidMount() {
        store.fetchRecords();
        store.fetchGitProjects();
        store.fetchJenkinsJobs();
            ...
            {hasPermission('deploy.app.config|deploy.app.edit') && (
                <Table.Column title="操作" render={info => (
                    <Action>
                        ...
                        <Action.Button auth="deploy.app.edit"
                                       onClick={() => this.handleDeployDelete(info)}>删除</Action.Button>
                    </Action>
                )}/>
            )}
        </Table>
                ...
                {hasPermission('deploy.app.edit|deploy.app.del') && (
                    <Table.Column width={260} title="操作" render={info => (
                        <Action>
                            ...
                            <Action.Button auth="deploy.app.edit"
                                           onClick={e => store.showForm(e, info)}>编辑</Action.Button>
                            <Action.Button auth="deploy.app.del"
                                           onClick={e => this.handleDelete(e, info)}>删除</Action.Button>
                        </Action>
                    )}/>
                )}
            </Table>
